...
function(x, ...){
  ...
  p <- p + theme(axis.text.x = element_blank(),axis.text.y = element_blank(),axis.ticks = element_blank()
      ...
      ,legend.position = c(0, 1), legend.justification=c(0,1) , legend.text=element_text(size=18)) + 
  ...
  p <- p + geom_text(data=dfids,aes(label=X1),hjust=1,vjust=0,angle = -22.5 , size=cex_size)#, color="dark gray")
  return(p)
}
